 ...
 * whenever possible.
 *
 * WebAudio pausing is accomplished by watching the page visibility API. When on
 * iOS, page focus is also used to determine if the page is in the foreground
 ...
 * foreground.
 *
 * iOS detection is done by looking at the user agent for iPhone, iPod, iPad.
 * This detects the phones fine, but Apple likes to pretend their new iPads are
 * computers (lol right..). Newer iPads are detected by finding Mac OSX in the
 * user agent and then checking for touch support by testing
 ...
 * someone out on Stack Overflow, it's just meant as an example.
 */
/**
 * Enables unmute.
 * @param context A reference to the web audio context to "unmute".
 * @param allowBackgroundPlayback Optional. Default false. Allows audio to
 * continue to play in the background. This is not recommended because it will
 * burn battery and display playback controls on the iOS lockscreen.
 * @param forceIOSBehavior Optional. Default false. Forces behavior to that which
 * is on iOS. This *could* be used in the event the iOS detection fails (which it
 * shouldn't). It is strongly recommended NOT to use this.
 * @returns An object containing a dispose function which can be used to dispose
 * of the unmute controller.
 */
